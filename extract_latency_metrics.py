...
def analyze_latency():
    ...
    for node in nodes.keys():
        log_file = f'/tmp/latency_{node}.log'
        try:
    ...
    # Display results
    print(f"{'Node':<6} {'Delay':<10} {'Loss':<8} {'Received':<12} {'Time(s)':<10} {'Failed':<8}")
    print("-" * 60)
    
    for node, data in results.items():
        received_str = "✓ Yes" if data['received'] else "✗ No"
        time_str = f"{data['time']:.2f}" if data['time'] else "N/A"
        print(f"{node:<6} {data['delay']:<10} {data['loss']:<8} {received_str:<12} {time_str:<10} {data['failed']:<8}")
    ...
    print(f"Total Failed Messages: {total_failed}")
    print(f"High-latency node (h3) received: {'✓ Yes' if results['h3']['received'] else '✗ No'}")
